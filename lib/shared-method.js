...
var Context = require('./context-base');
var numberChecks = require('./number-checks');

var isInteger = numberChecks.isSafeInteger;
var isSafeInteger = numberChecks.isSafeInteger;
...
SharedMethod.prototype.invoke = function(scope, args, remotingOptions, ctx, cb) {
  ...
  if (accepts) {
    for (var i = 0; i < accepts.length; i++) {
      ...
      var coercionOptions = {};
      if (accepts[i].allowArray) {
        coercionOptions.allowArray = accepts[i].allowArray;
      }
      try {
        uarg = validateInputArgument(uarg, desc, ctx, coercionOptions);
      } catch (e) {
...
function validateInputArgument(uarg, desc, ctx, options) {
  ...
  if (desc.required) {
    var argIsNotSet = uarg === null || uarg === undefined || uarg === '';
    if (argIsNotSet) {
      throw badArgumentError(g.f('%s is a required argument', name));
    }
  }

  var converter = ctx.typeRegistry.getConverter(desc.type);
  var err = converter.validate(ctx, uarg, options);
  if (err) {
